...
// Fungsi untuk membuat/update tabel sudah_kembali_dari_service
function createTableSudahKembali($koneksi) {
    $query = "CREATE TABLE IF NOT EXISTS sudah_kembali_dari_service (
        ...
        kondisi_setelah_service ENUM('baik', 'rusak_ringan', 'tidak_bisa_diperbaiki') DEFAULT 'baik',
...
createTableSudahKembali($koneksi);
...
// FUNGSI 2: Fungsi universal untuk menyimpan data
function saveDataBarangUniversal($koneksi, $data, $foto = null) {
    // Tentukan tabel target berdasarkan status_barang
    $table_name = '';
    $default_status = '';
    ...
    switch($data['status_barang']) {
        case 'belum_kembali':
            $table_name = 'belum_kembali_dari_service';
            $default_status = 'belum_kembali';
            $is_service_table = true;
            break;
        case 'sudah_kembali':
            $table_name = 'sudah_kembali_dari_service';
            $default_status = 'sudah_kembali';
            $is_service_table = true;
            break;
        case 'di_ruangan':
            $table_name = 'barang_di_ruangan';
            $default_status = 'di_ruangan';
            $is_service_table = false;
            break;
        case 'di_gudang':
            $table_name = 'barang_di_gudang';
            $default_status = 'di_gudang';
            $is_service_table = false;
            break;
        default:
            return false;
    ...
    // Cek apakah tabel exists, jika tidak buat tabel
    if (!createTableIfNotExists($koneksi, $table_name, $default_status)) {
        error_log("Gagal membuat atau memeriksa tabel: $table_name");
        return false;
    }
    
    // Auto-generate nomor urut untuk tabel yang dipilih
    $no_urut = getNextNoUrutUniversal($koneksi, $table_name);
    ...
    if ($is_service_table) {
        ...
        $kondisi_setelah_service = mysqli_real_escape_string($koneksi, $data['kondisi_setelah_service'] ?? 'baik');
        ...
        if (!empty($id)) {
            ...
            $query = "UPDATE $table_name SET 
                ...
                kondisi_setelah_service = '$kondisi_setelah_service',
    ...
    // Untuk tabel barang di ruangan/gudang - handle kedua nama field
    $kondisi = mysqli_real_escape_string($koneksi, $data['kondisi_setelah_service'] ?? $data['kondisi'] ?? 'baik');
...
function getNextNoUrut($koneksi) {
    $query = "SELECT MAX(no_urut) as max_urut FROM sudah_kembali_dari_service";
    $result = mysqli_query($koneksi, $query);
...
// Fungsi untuk mendapatkan data sudah kembali dari service
function getSudahKembaliService($koneksi) {
    $query = "SELECT * FROM sudah_kembali_dari_service ORDER BY no_urut ASC";
    $result = mysqli_query($koneksi, $query);
...
// Fungsi untuk menyimpan data ke tabel sudah_kembali_dari_service
function saveSudahKembaliService($koneksi, $data, $foto = null) {
    // Escape semua input
    $id = isset($data['id']) ? intval($data['id']) : 0;
    ...
    if (!empty($id)) {
        // UPDATE data
        $query = "UPDATE sudah_kembali_dari_service SET 
            no_urut = '$no_urut',
            nama_barang = '$nama_barang',
            ruangan = '$ruangan',
            merek = '$merek',
        ...
        // INSERT data baru
        $query = "INSERT INTO sudah_kembali_dari_service 
                  (no_urut, nama_barang, ruangan, merek, tempat_service, tanggal_diambil_dari_ruangan, 
                   ...
                   tanggal_kembali, kondisi_setelah_service)
                  VALUES (
                  '$no_urut',
                  '$nama_barang',
                  '$ruangan',
...
function deleteSudahKembaliService($koneksi, $id) {
    // Hapus file foto jika ada
    $data = getSudahKembaliById($koneksi, $id);
    ...
    $id = mysqli_real_escape_string($koneksi, $id);
    $query = "DELETE FROM sudah_kembali_dari_service WHERE id = $id";
    return mysqli_query($koneksi, $query);
}

// Fungsi untuk mendapatkan data by ID
function getSudahKembaliById($koneksi, $id) {
    return getDataByIdUniversal($koneksi, $id, 'sudah_kembali');
}

// Fungsi untuk mendapatkan statistik
function getStatistikSudahKembali($koneksi) {
    $statistik = [
        'total' => 0,
        'baik' => 0,
        'rusak_ringan' => 0,
    ...
    try {
        $query_total = "SELECT COUNT(*) as total FROM sudah_kembali_dari_service";
        $result_total = mysqli_query($koneksi, $query_total);
        if ($result_total) {
            $row = mysqli_fetch_assoc($result_total);
            $statistik['total'] = $row['total'] ?? 0;
        }
        
        $query_kondisi = "SELECT kondisi_setelah_service, COUNT(*) as jumlah 
                         FROM sudah_kembali_dari_service 
                         GROUP BY kondisi_setelah_service";
        $result_kondisi = mysqli_query($koneksi, $query_kondisi);
        
        if ($result_kondisi) {
            while ($row = mysqli_fetch_assoc($result_kondisi)) {
                switch ($row['kondisi_setelah_service']) {
                    case 'baik':
                        $statistik['baik'] = $row['jumlah'];
                        break;
                    case 'rusak_ringan':
                        $statistik['rusak_ringan'] = $row['jumlah'];
                        break;
                    case 'tidak_bisa_diperbaiki':
                        $statistik['tidak_bisa_diperbaiki'] = $row['jumlah'];
                        break;
                }
            }
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['simpan'])) {
        ...
        $data = [
            ...
            'kondisi_setelah_service' => $_POST['kondisi_setelah_service'] ?? 'baik'
        ];
...
if (isset($_GET['hapus'])) {
    $id = $_GET['hapus'];
    if (is_numeric($id)) {
        if (deleteSudahKembaliService($koneksi, $id)) {
...
$data_sudah_kembali = getSudahKembaliService($koneksi);
$statistik = getStatistikSudahKembali($koneksi);

// Cek apakah sedang edit atau lihat detail
$edit_data = null;
if (isset($_GET['edit']) && is_numeric($_GET['edit'])) {
    $edit_data = getDataByIdUniversal($koneksi, $_GET['edit'], 'sudah_kembali');
}

$detail_data = null;
if (isset($_GET['detail']) && is_numeric($_GET['detail'])) {
    $detail_data = getSudahKembaliById($koneksi, $_GET['detail']);
}
...
<head>
    ...
    <title>Dashboard - Data Sudah Kembali dari Service</title>
    ...
    * {
    ...
    .progress-chart {
        ...
        height: 200px;
        border-radius: 50%;
        background: conic-gradient(#4caf50 0% <?=$statistik['presentase_baik'] ?>%,
                #ff9800 <?=$statistik['presentase_baik'] ?>% <?=$statistik['presentase_baik'] + $statistik['presentase_rusak_ringan'] ?>%,
                #f44336 <?=$statistik['presentase_baik'] + $statistik['presentase_rusak_ringan'] ?>% 100%);
        position: relative;
        display: flex;
        align-items: center;
        justify-content: center;
    ...
    <div id="content">
        ...
        <main>
            <div class="head-title">
                <div class="left">
                    <h1>Data Barang Sudah Kembali dari Service</h1>
                    <ul class="breadcrumb">
                        <li>
                            <a href="#">Dashboard</a>
                        </li>
                        ...
                        <li>
                            <a href="#" class="active">Sudah Kembali</a>
                        </li>
                    </ul>
                </div>
            ...
            <div class="table-data">
                <div class="order">
                    <div class="head">
                        <h3>Data Barang Sudah Kembali dari Service</h3>
                        <div>
                    ...
                    <?php if (empty($data_sudah_kembali)): ?>
                    <div class="no-data">
                        <i class="fas fa-inbox"></i>
                        <h3>Tidak ada data barang yang sudah kembali</h3>
                    ...
                    <div style="overflow-x: auto;">
                        <table>
                            ...
                            <tbody>
                                <?php foreach ($data_sudah_kembali as $item): ?>
                                <tr>
                                    <td><?= $item['no_urut'] ?></td>
                                    ...
                                    <td>
                                        <span class="status <?= $item['kondisi_setelah_service'] ?>">
                                            <?= 
                                                $item['kondisi_setelah_service'] == 'baik' ? 'Baik' : 
                                                ($item['kondisi_setelah_service'] == 'rusak_ringan' ? 'Rusak Ringan' : 'Tidak Bisa Diperbaiki')
                                            ?>
                                        </span>
                                    </td>
                                    <td>
            ...
            <div class="form-container">
                ...
                <form method="POST" action="" enctype="multipart/form-data">
                    ...
                    <input type="hidden" name="no_urut" value="<?= $edit_data['no_urut'] ?? $next_no_urut ?>">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="status_barang" name="status_barang" required>
                            ...
                            <option value="belum_kembali"
                                <?= (isset($edit_data['status_barang']) && $edit_data['status_barang'] == 'belum_kembali') ? 'selected' : '' ?>>
                                Belum Kembali dari Service</option>
                            <option value="sudah_kembali"
                                <?= (isset($edit_data['status_barang']) && $edit_data['status_barang'] == 'sudah_kembali') ? 'selected' : (isset($_GET['tambah']) ? 'selected' : '') ?>>
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            <label for="kondisi_setelah_service">Kondisi Setelah Service *</label>
                            <select class="form-control" id="kondisi_setelah_service" name="kondisi_setelah_service"
                                required>
                                <option value="baik"
                                    <?= (isset($edit_data['kondisi_setelah_service']) && $edit_data['kondisi_setelah_service'] == 'baik') ? 'selected' : '' ?>>
                                    Baik</option>
                                <option value="rusak_ringan"
                                    <?= (isset($edit_data['kondisi_setelah_service']) && $edit_data['kondisi_setelah_service'] == 'rusak_ringan') ? 'selected' : '' ?>>
                                    Rusak Ringan</option>
                                <option value="tidak_bisa_diperbaiki"
                                    <?= (isset($edit_data['kondisi_setelah_service']) && $edit_data['kondisi_setelah_service'] == 'tidak_bisa_diperbaiki') ? 'selected' : '' ?>>
                    ...
                    <div class="form-group">
                        ...
                        <div class="current-photo" style="margin-top: 10px;">
                            <p>Foto Saat Ini:</p>
                            <img src="uploads/<?= $edit_data['bukti_foto'] ?>" alt="Bukti Foto"
                    ...
                    <div class="form-group">
                        ...
                        <a href="ds_kembali.php" class="btn-custom btn-secondary">
            ...
            <div class="container-detail">
                ...
                <div class="detail-item">
                    ...
                    <div class="detail-card">
                        <div class="label">Kondisi Setelah Service</div>
                        <div class="value">
                            <span class="status <?= $detail_data['kondisi_setelah_service'] ?>">
                                <?= 
                                    $detail_data['kondisi_setelah_service'] == 'baik' ? 'Baik' : 
                                    ($detail_data['kondisi_setelah_service'] == 'rusak_ringan' ? 'Rusak Ringan' : 'Tidak Bisa Diperbaiki')
                                ?>
                            </span>
                        </div>
                    </div>
                ...
                <a href="ds_kembali.php" class="btn-back">
