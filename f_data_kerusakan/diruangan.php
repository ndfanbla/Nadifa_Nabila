...
// Fungsi untuk membuat/update tabel barang_di_ruangan
function createTableBarangRuangan($koneksi) {
    $query = "CREATE TABLE IF NOT EXISTS barang_di_ruangan (
...
createTableBarangRuangan($koneksi);
...
// ==================== FUNGSI KHUSUS BARANG DI RUANGAN ====================

// Fungsi untuk mendapatkan nomor urut berikutnya
function getNextNoUrut($koneksi) {
    $query = "SELECT MAX(no_urut) as max_urut FROM barang_di_ruangan";
    $result = mysqli_query($koneksi, $query);
...
// Fungsi untuk mendapatkan data barang di ruangan
function getBarangDiRuangan($koneksi) {
    $query = "SELECT * FROM barang_di_ruangan ORDER BY no_urut ASC";
    $result = mysqli_query($koneksi, $query);
...
// Fungsi untuk menyimpan data barang di ruangan
function saveBarangDiRuangan($koneksi, $data, $foto = null) {
    // Escape semua input
    ...
    if (!empty($id)) {
        // UPDATE data
        $query = "UPDATE barang_di_ruangan SET 
            no_urut = '$no_urut',
            nama_barang = '$nama_barang',
            ruangan = '$ruangan',
            merek = '$merek',
        ...
        // INSERT data baru
        $query = "INSERT INTO barang_di_ruangan 
                  (no_urut, nama_barang, ruangan, merek, nomor_barang, kondisi, keterangan, bukti_foto)
                  VALUES (
                  '$no_urut',
                  '$nama_barang',
                  '$ruangan',
...
function deleteBarangDiRuangan($koneksi, $id) {
    // Hapus file foto jika ada
    $data = getBarangDiRuanganById($koneksi, $id);
    ...
    $id = mysqli_real_escape_string($koneksi, $id);
    $query = "DELETE FROM barang_di_ruangan WHERE id = $id";
    return mysqli_query($koneksi, $query);
}

// Fungsi untuk mendapatkan data by ID
function getBarangDiRuanganById($koneksi, $id) {
    return getDataByIdUniversal($koneksi, $id, 'di_ruangan');
}

// Fungsi untuk mendapatkan statistik
function getStatistikBarangRuangan($koneksi) {
    $statistik = [
        'total' => 0,
        'baik' => 0,
        'rusak_ringan' => 0,
    ...
    try {
        $query_total = "SELECT COUNT(*) as total FROM barang_di_ruangan";
        ...
        $query_kondisi = "SELECT kondisi, COUNT(*) as jumlah 
                         FROM barang_di_ruangan 
                         GROUP BY kondisi";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['simpan'])) {
        ...
        $required_fields = ['nama_barang', 'ruangan', 'merek', 'status_barang'];
...
if (isset($_GET['hapus'])) {
    $id = $_GET['hapus'];
    if (is_numeric($id)) {
        if (deleteBarangDiRuangan($koneksi, $id)) {
...
$data_barang_ruangan = getBarangDiRuangan($koneksi);
$statistik = getStatistikBarangRuangan($koneksi);

// Cek apakah sedang edit atau lihat detail
$edit_data = null;
if (isset($_GET['edit']) && is_numeric($_GET['edit'])) {
    $edit_data = getDataByIdUniversal($koneksi, $_GET['edit'], 'di_ruangan');
}

$detail_data = null;
if (isset($_GET['detail']) && is_numeric($_GET['detail'])) {
    $detail_data = getBarangDiRuanganById($koneksi, $_GET['detail']);
}
...
<head>
    ...
    <title>Dashboard - Barang di Ruangan</title>
        ...
        <main>
            <div class="head-title">
                <div class="left">
                    ...
                    <ul class="breadcrumb">
                        ...
                        <li>
                            <a href="#" class="active">Barang di Ruangan</a>
                        </li>
                    </ul>
                </div>
            ...
            <div class="table-data">
                <div class="order">
                    ...
                    <?php if (empty($data_barang_ruangan)): ?>
                    <div class="no-data">
                        <i class="fas fa-inbox"></i>
                        <h3>Tidak ada data barang di ruangan</h3>
                    ...
                    <div style="overflow-x: auto;">
                        <table>
                            ...
                            <tbody>
                                <?php foreach ($data_barang_ruangan as $item): ?>
                                <tr>
                                    <td><?= $item['no_urut'] ?></td>
                                    <td><?= htmlspecialchars($item['nama_barang']) ?></td>
                                    <td><?= htmlspecialchars($item['ruangan']) ?></td>
                                    <td><?= htmlspecialchars($item['merek']) ?></td>
                                    <td><?= !empty($item['nomor_barang']) ? htmlspecialchars($item['nomor_barang']) : '-' ?>
                                    </td>
                                    <td>
                                        <span class="status <?= $item['kondisi'] ?>">
                                            <?= 
                                                $item['kondisi'] == 'baik' ? 'Baik' : 
                                                ($item['kondisi'] == 'rusak_ringan' ? 'Rusak Ringan' : 'Tidak Bisa Diperbaiki')
                                            ?>
                                        </span>
                                    </td>
                                    <td>
            ...
            <div class="form-container">
                ...
                <form method="POST" action="" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="status_barang" name="status_barang" required>
                            ...
                            <option value="di_ruangan"
                                <?= (isset($edit_data['status_barang']) && $edit_data['status_barang'] == 'di_ruangan') ? 'selected' : (isset($_GET['tambah']) ? 'selected' : '') ?>>
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" id="kondisi" name="kondisi" required>
                                <option value="baik"
                                    <?= (isset($edit_data['kondisi']) && $edit_data['kondisi'] == 'baik') ? 'selected' : '' ?>>
                                    Baik</option>
                                <option value="rusak_ringan"
                                    <?= (isset($edit_data['kondisi']) && $edit_data['kondisi'] == 'rusak_ringan') ? 'selected' : '' ?>>
                                    Rusak Ringan</option>
                                <option value="tidak_bisa_diperbaiki"
                                    <?= (isset($edit_data['kondisi']) && $edit_data['kondisi'] == 'tidak_bisa_diperbaiki') ? 'selected' : '' ?>>
                    ...
                    <div class="form-group">
                        ...
                        <a href="diruangan.php" class="btn-custom btn-secondary">
            ...
            <div class="container-detail">
                ...
                <div class="detail-item">
                    ...
                    <div class="detail-card">
                        ...
                        <div class="value">
                            <span class="status <?= $detail_data['kondisi'] ?>">
                                <?= 
                                    $detail_data['kondisi'] == 'baik' ? 'Baik' : 
                                    ($detail_data['kondisi'] == 'rusak_ringan' ? 'Rusak Ringan' : 'Tidak Bisa Diperbaiki')
                                ?>
                            </span>
                        </div>
                    </div>
                ...
                <a href="diruangan.php" class="btn-back">
